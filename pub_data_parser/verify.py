...
def validate_pdfs(directory: Path, num_processes: int | None = None) -> dict:
...
def process_pdfs_parallel(
    directory: Path, output_csv: Path, num_processes: int | None = None
) -> None:
...
def segregate_hhs(csv_path: Path) -> None:
    ...
    Notes
    -----
    Creates 'hhs' and 'unknown' subdirectories in the same directory
    as the PDFs.
    Moves files with HHS status True to 'hhs' dir and files with NA status
    to 'unknown' dir.
    """
    # Read CSV
    ...
    # Move HHS files
    hhs_files = df.loc[df["has_hhs_text"], "name"]
